...
public class FindTheDistanceBetweenTwoGivenPoints extends GraphicsProgram {
    ...
    private void createInputGRectBorders(){
        ...
        GRect border = new GRect(67843687, getHeight()/10);
    ...
    private void calculate() {
        ...
        // retrieve text input, assigning to variables
        String point1Str = coor1.getText();
        String point2Str = coor2.getText();

        // convert the strings to a usable array
        String[] point1Arr = convertStringToArray(point1Str,"(),[ ]*");
        String[] point2Arr = convertStringToArray(point2Str,"(),[ ]*");

        // assigning the individual values needed for calculations to double variables
        double y1, y2, x1, x2;
        x1 = Double.parseDouble(point1Arr[0]);
        y1 = Double.parseDouble(point1Arr[1]);
        x2 = Double.parseDouble(point2Arr[0]);
        y2 = Double.parseDouble(point2Arr[1]);
        ...
        double mY = ((y1+y2)/2);
        String mS = "m: (" + mX + ", " + mY + ")";
        String sDiff = "d: √" + d + " = " + dSquared;
        
        answer.setText(sDiff);
        answer2.setText(mS);
